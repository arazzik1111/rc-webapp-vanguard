...
export class HttpStore extends BaseStore {
  ...
  getHeaders(isFormData = false, token: string = '') {
    ...
    // return {
    //   // 'Authorization': `Bearer ${cookieService.get(CookieKeys.TOKEN)}`,
    //   // "x-api-key": "",
    // };
    return Headers;
  }
  ...
  get<T extends BaseModel | any>(
    ...
    const { host, token, locationId, widgetId } = instance.get();
    if (host) {
      endPoint.base = host;
    }
  ...
  post(
    ...
    const options: AxiosRequestConfig = {
      ...
      onUploadProgress: (progress) => {
        console.log(`${((progress.loaded / progress.total) * 100).toFixed(2)}% - ${progress.total / (1024 * 1024)}MB`);
      },
    };
    return Axios.request(options).pipe(
      map((resp) => {
