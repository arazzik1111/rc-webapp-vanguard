...
export const WithFooterBorder: Story = {
  args: {
    title: "Modal with Footer Border",
    children: <div>Content with footer border</div>,
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText("Modal with Footer Border")).toBeInTheDocument();

    // Close the modal
    const cancelButton = canvas.getByRole('button', { name: /cancel/i });
    await userEvent.click(cancelButton);
    await wait(0)
    await expect(canvas.queryByText("Modal with Footer Border")).not.toBeInTheDocument();
  },
  render: (args) => {
    const openModal = () => {
      ModalService.open(
        <EditModal
          ...
          showFooterTopBorder={args.showFooterTopBorder}
