...
export const WithButtonIcon: Story = {
  args: {
    title: "Save Document",
    children: <div>Document content</div>,
    showPositiveBtnIcon: true,
    savable: true,
  ...
  play: async ({ canvasElement }) => {
    ...
    // Verify save button has icon (check for icon element or aria-label)
    ...
    await wait(0)
    await expect(canvas.queryByText("Save Document")).not.toBeInTheDocument();
  },
  render: (args) => {
    const openModal = () => {
      ModalService.open(
        <EditModal
          ...
          showPositiveBtnIcon={args.showPositiveBtnIcon}
