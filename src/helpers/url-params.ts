...
/**
 * Used to mock adding of ad account
 * Can be extended to any other mocking we need
 * */
export const IS_TEST_FLOW_QP = 'isTestFlow';
/**
 *
 * Passing a local ref object and an ulr param we set the localvalue to the url param
 * */
export const setLocalToUrl = (local: MutableRefObject<HTMLInputElement> | null, urlName: string): string => {
  ...
  // if we have a url param we set that as the real one
  if (urlParam) {
    searchable = urlParam;
