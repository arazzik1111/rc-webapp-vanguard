...
export const highlightedTextMaxLength = (content: string, maxLength: number, className: string) => {
  if (content.length > maxLength) {
    ...
    const normalText = content.slice(0, Math.max(0, safeMaxLength));
    const highlightedText = content.slice(Math.max(0, safeMaxLength));
...
export const replaceAndWithAmpersantSymbol = (input: string): string => {
  return input.replace(/\b(And|and)\b/g, '&');
};
...
export const encodeStringForHTMLAttribute = (str: string) => {
  const bytes = new TextEncoder().encode(str);
  const binary = String.fromCharCode(...bytes);
  return btoa(binary);
};

export const decodeStringFromHTMLAttribute = (str: string) => {
